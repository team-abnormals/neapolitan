...
public class NeapolitanBiomeFeatures {
	...
	public static void onBiomeLoad(BiomeLoadingEvent event) {
		...
		if (category.equals(Biome.Category.PLAINS))
			event.getGeneration().withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Configured.PATCH_STRAWBERRY_BUSH);

		if (category.equals(Biome.Category.SAVANNA))
			event.getGeneration().withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Configured.PATCH_VANILLA_VINE);

		if (event.getClimate().precipitation.equals(RainType.RAIN)) {
			if (category.equals(Biome.Category.BEACH))
				event.getGeneration().withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Configured.BANANA_PLANT_BEACH);
			...
			if (category.equals(Biome.Category.JUNGLE)) {
				if (!DataUtil.matchesKeys(biome, Biomes.BAMBOO_JUNGLE) && !DataUtil.matchesKeys(biome, Biomes.BAMBOO_JUNGLE_HILLS)) {
					if (!DataUtil.matchesKeys(biome, Biomes.JUNGLE_EDGE) && !DataUtil.matchesKeys(biome, Biomes.MODIFIED_JUNGLE_EDGE))
						event.getGeneration().withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Configured.BANANA_PLANT_JUNGLE);
					else
						event.getGeneration().withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Configured.BANANA_PLANT_RARE);
				}
			}
			...
			if (biome.getPath().contains("rainforest")) {
				if (BiomeDictionary.hasType(key, BiomeDictionary.Type.FOREST))
					...
					event.getGeneration().withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Configured.BANANA_PLANT_VERY_RARE);
			}
		}
	}
}
