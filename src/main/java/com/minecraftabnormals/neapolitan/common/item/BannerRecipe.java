...
/*
 * @author Vazkii - Botania
 * https://github.com/Vazkii/Botania/blob/master/src/main/java/vazkii/botania/common/block/ModBanners.java
 */
public class BannerRecipe extends SpecialRecipe {
	...
	public boolean matches(CraftingInventory inv, @Nonnull World world) {
		...
		for (int i = 0; i < inv.getSizeInventory(); i++) {
			...
			if (NeapolitanBanners.PATTERNS.containsKey(item.delegate) && !foundItem) {
				foundItem = true;
	...
	public ItemStack getCraftingResult(@Nonnull CraftingInventory inv) {
		...
		for (int i = 0; i < inv.getSizeInventory(); i++) {
			ItemStack stack = inv.getStackInSlot(i);
			Item item = stack.getItem();
			if (NeapolitanBanners.PATTERNS.containsKey(item.delegate)) {
				pattern = NeapolitanBanners.PATTERNS.get(item.delegate);
