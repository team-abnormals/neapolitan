...
public class NeapolitanLootModifierProvider extends LootModifierProvider {
	...
	public NeapolitanLootModifierProvider(PackOutput output, CompletableFuture<Provider> provider) {
	...
	protected void registerEntries(Provider provider) {
		...
		this.entry("ice").selects("blocks/ice").addModifier(new LootPoolsModifier(Collections.singletonList(LootPool.lootPool().name("neapolitan:ice").setRolls(ConstantValue.exactly(1.0F)).add(LootItem.lootTableItem(NeapolitanItems.ICE_CUBES.get()).apply(SetItemCountFunction.setCount(UniformGenerator.between(0, 2))).apply(ApplyBonusCount.addUniformBonusCount(Enchantments.BLOCK_FORTUNE)).apply(ApplyExplosionDecay.explosionDecay())).when(MatchTool.toolMatches(ItemPredicate.Builder.item().of(ItemTags.PICKAXES))).when(MatchTool.toolMatches(Builder.item().hasEnchantment(new EnchantmentPredicate(Enchantments.SILK_TOUCH, Ints.atLeast(1)))).invert()).build()), false));
	}
